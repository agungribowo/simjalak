...
//session admin 
	...
	function getMapel($idmapel) { 
		$CI =& get_instance(); 
		$ambil = $CI->db->query("SELECT namamapel FROM tbl_matapelajaran WHERE idmapel = '$idmapel'")->row(); 
...
//getLab 
	function getLab($idlab) { 
		...
		return $ambil->ruanglab; 
	} 
 
//getGuru 
	function getGuru($idguru) { 
		$CI =& get_instance(); 
		$ambil = $CI->db->query("SELECT namaguru FROM tbl_guru WHERE idguru = '$idguru'")->row(); 
